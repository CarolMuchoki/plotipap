...
<head>
    ...
    <script>
        ...
        function buyPlots() {
            ...
            var selectedList = getSelectedList('Buy Plots');

            if (selectedList) {
                if (confirm('Do you want to buy selected plot/s ' 
                             + selectedList + '?')) {
                    document.forms[0].oldStatusCode.value=0;
                    document.forms[0].newStatusCode.value=1;
                    document.forms[0].action='buy.php';
        ...
        function cancelPlots() {

            var selectedList = getSelectedList('Cancel Buying');

            if (selectedList) {
                if (confirm('Do you want to cancel selected plot/s ' + selectedList + '?')) {
        ...
        function confirmSeats() {
            ...
            var selectedList = getSelectedList('Confirm Buying');

            if (selectedList) {
                if (confirm('Do you want to confirm selected plot/s ' + selectedList + '?')) {
                    document.forms[0].oldStatusCode.value=1;
        ...
        function getSelectedList(actionSelected) {
            ...
            for (var i = 0; i < obj.length; i++) {
                if (obj[i].checked && obj[i].name == 'plots[]') {
            ...
            // no selection error
            if (selectedList == '') {
                alert('Please select a section before clicking ' + actionSelected);
                return false;
            } else {
                return selectedList;
            }
